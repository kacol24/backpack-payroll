        ...
        <div class="{{ $crud->getListContentClass() }}">
            ...
            <div class="row">
                ...
                @php
                    ...
                    $sumNetpay = App\Models\Payslip::when($year, function($query, $year){
                                                        foreach (json_decode($year) as $index => $year) {
                                                            if ($index == 0) {
                                                                $query->whereYear('period', $year);
                                                            } else {
                                                                $query->orWhereYear('period', $year);
                                                            }
                                                        }
                                                    ...
                                                    ->when($month, function($query, $month){
                                                        foreach (json_decode($month) as $index => $month) {
                                                            if ($index == 0) {
                                                                $query->whereMonth('period', $month);
                                                            } else {
                                                                $query->orWhereMonth('period', $month);
                                                            }
                                                        }
                        ...
                        renderer: function(api, rowIdx, columns) {
                            ...
                            var data = $.map(columns, function(col, i) {
                                ...
                                return '<tr data-dt-row="' + col.rowIndex + '" data-dt-column="' + col.columnIndex +
                                    '">' +
                                    '<td style="vertical-align:top; border:none;"><strong>' + col.title.trim() + ':' +
                                    '<strong></td> ' +
                                    '<td style="padding-left:10px;padding-bottom:10px; border:none;">' + col.data +
                                    '</td>' +
                                    '</tr>';
                            }).join('');
